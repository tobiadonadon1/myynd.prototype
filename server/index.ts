...
/** Gli accessi a Granola già detti alle altre finestre: il «fatto» si dice una volta. */
const granolaDetti = new Set<string>()
app.get('/api/connettori/granola/avvia/:id', (req, res) => {
  ...
  // la finestra che aspetta lo sa da questa risposta; le altre (il richiamo,
  // un'altra scheda) lo sanno dal filo, come per ogni altro collegamento
  if (s.stato === 'fatto' && !granolaDetti.has(req.params.id)) {
    granolaDetti.add(req.params.id)
    compiti.annunciaCollegamento()
  }
  res.json(s)
})

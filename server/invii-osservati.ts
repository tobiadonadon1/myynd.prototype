...
// Fra più mail partite nel filo si sceglie, non si prende la prima: quella
// con il Message-ID della bozza, o la più vicina alla bozza. Una riga sua
// corta mandata prima («arrivo, te li mando») non nasconde l'invio vero, e
// una «propria» segnata a un giro cede il posto alla bozza vista partire dopo.
 ...
 * senza un invio già scritto nelle misure (da «Manda»), degli ultimi trenta
 * giorni. Una risposta sua vista al giro prima («propria») non chiude la
 * partita: la bozza può partire dopo, e allora vale quella.
 */
function candidati(): store.Compito[] {
  ...
  const righe = store.default.prepare(`
    ...
    WHERE c.email LIKE '%"stato":"salvata"%' AND c.doc IS NOT NULL AND c.mandata IS NULL AND c.sparito IS NULL
      AND c.chiesto IS NOT NULL AND c.chiesto >= ? AND (m.inviato IS NULL OR m.via = 'propria')
      AND NOT EXISTS (
...
export async function osservaUno(c: store.Compito): Promise<Visto> {
  ...
  // una risposta sua vista prima è una lettura del filo, non un fatto sulla
  // bozza: se poi la bozza parte davvero, vale quella e prende il suo posto
  const propria = m?.via === 'propria'
  // un invio c'è già, da «Manda»: la copia nella posta inviata è quella mail, non una seconda
  if (m?.inviato && !propria) return null
  ...
  const idBozza = idDellaBozza(c.id, c.doc)
  // fra più mail partite si sceglie: quella con il Message-ID della bozza, o
  // la più vicina alla bozza. La prima in ordine di tempo può essere una riga
  // sua («arrivo, te li mando») scritta prima di mandare la bozza
  const viste = mandate.map(d => {
    const corpo = senzaFirma(corpoAttuale(d))
    return { d, corpo, conId: !!d.messageId && d.messageId === idBozza, r: ritocco(bozza, corpo) }
  })
  const scelta = viste.find(v => v.conId) ?? viste.reduce((a, b) => (b.r < a.r ? b : a))
  const { d: inviata, corpo, r } = scelta
  const certezza: 'id' | 'filo' = scelta.conId ? 'id' : 'filo'

...
export function perIlModello(): string {
  return vivi().slice(0, PER_IL_MODELLO)
    .map(p => `— ${p.nome}${p.obiettivo ? `: ${p.obiettivo}` : ''} (${p.stato})`)
    .join('\n')
}
...
export function tocca(p: { nome: string; obiettivo: string }, testo: string): boolean {
  const piano = testo.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
  const nome = p.nome.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim()
  if (nome.length >= 3 && piano.includes(nome)) return true
  const parole = paroleDi(p)
  if (parole.length < 2) return false
  let trovate = 0
  for (const w of parole) {
    if (piano.includes(w) && ++trovate >= 2) return true
  }
  return false
}

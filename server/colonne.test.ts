...
test('un database nuovo arriva allo schema 66, una voce per migrazione', () => {
  ...
  assert.equal(schema.migrazioni, 66)
...
test('un indice vero fermo alla 47 arriva alla 64 senza perdere niente', () => {
  ...
  store.chiudiIndici()
  assert.equal((store.default.prepare('PRAGMA user_version').get() as { user_version: number }).user_version, 66)
})

  ...
  finto = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (req.url?.includes('/chat/completions')) {
        return res.end(JSON.stringify({ id: 'c1', object: 'chat.completion', model: 'finto', choices: [{ index: 0, message: { role: 'assistant', content: JSON.stringify({ voci: [] }) }, finish_reason: 'stop' }], usage: { prompt_tokens: 1, completion_tokens: 1 } }))
      }
...
test('il giro di fondo sul passo delle fonti non consuma la prima pagina; «Leggi» la fa', async () => {
  ...
  // il giro di fondo legge il Mac, e la pagina non parte
  ...
  assert.match(registro, /prima pagina · pronta in \d+ ms · \d+ carte · la pagina \d+ ms/)
})

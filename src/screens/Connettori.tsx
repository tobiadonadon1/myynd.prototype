    ...
    <div style={{ width: 700, maxWidth: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, padding: '12px 4px 20px' }}>
        ...
        {v.connCount > 0 && (
          ...
          }}>{v.sincronizzando ?? 'Rileggi tutto'}</button>
        )}
      </div>
      ...
      {v.connSpenti.length > 0 && (
        <>
          ...
          <div style={{ flex: 'none', display: 'flex', flexWrap: 'wrap', gap: 9 }}>
            {v.connSpenti.map(c => (
              <Hov key={c.id} as="button" onClick={c.onClick} title={c.nota}
      ...
      <div style={{ ...LABEL, color: 'rgba(34,39,31,.5)', padding: '24px 4px 10px' }}>Più avanti</div>
      ...
      <div style={{ fontSize: '12.5px', color: 'rgba(34,39,31,.5)', lineHeight: 1.6, padding: '14px 4px 0', maxWidth: 520 }}>
        Questi hanno bisogno di un'app registrata o di un OAuth: restano qui
        perché fanno parte del disegno, ma non sono ancora collegabili.
      </div>
    </div>
  )
}

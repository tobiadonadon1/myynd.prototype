...
function statoInFondo(a: Automazione): { testo: string; tinta: string; pallino: string } {
  const ROSSO = 'var(--rame-testo)'
  const AMBRA = 'var(--ambra)'
  const VERDE = 'rgba(var(--salvia-rgb),.758)'
  const SPENTO = 'rgba(var(--inchiostro-rgb),.2)'

  if (!a.accesa) return { testo: t('In pausa'), tinta: 'rgba(var(--inchiostro-rgb),.5)', pallino: SPENTO }
  ...
  switch (a.salute.stato) {
    ...
    case 'ferma':
      return { testo: t('aspetta che chiudi la sua riga'), tinta: 'rgba(var(--inchiostro-rgb),.55)', pallino: AMBRA }
    default:
      return {
        ...
        tinta: 'rgba(var(--inchiostro-rgb),.5)',
        pallino: VERDE
      }
  }
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: a.accesa
          ? 'linear-gradient(165deg,rgba(var(--carta-rgb),.86),rgba(var(--carta-rgb),.7))'
          : 'linear-gradient(165deg,rgba(var(--carta-rgb),.6),rgba(var(--carta-rgb),.46))',
        backdropFilter: 'blur(26px) saturate(1.45)', WebkitBackdropFilter: 'blur(26px) saturate(1.45)',
        border: `1px solid ${sopra ? 'rgba(var(--luce-rgb),.98)' : 'rgba(var(--luce-rgb),.78)'}`,
        boxShadow: presa
          ? '0 4px 12px -6px rgba(var(--ombra-rgb),.3)'
          : sopra
            ? '0 26px 52px -20px rgba(var(--ombra-rgb),.34), inset 0 1px 0 rgba(var(--luce-rgb),.7)'
            : '0 14px 32px -18px rgba(var(--ombra-rgb),.26), inset 0 1px 0 rgba(var(--luce-rgb),.55)',
      ...
      */}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 9, minHeight: 22 }}>
        <div style={{
          flex: 1, minWidth: 0,
          fontSize: '15px', fontWeight: 500, color: 'var(--inchiostro)', letterSpacing: '-.012em',
      ...
      <div style={{
        fontSize: '12px', lineHeight: 1.5, color: 'rgba(var(--inchiostro-rgb),.55)', flex: 1,
      ...
      */}
      {!!suoi.length && (
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', margin: '11px 0 0' }}>
          ...
          {suoi.length > 3 && (
            <span style={{ fontSize: '10.5px', color: 'rgba(var(--inchiostro-rgb),.4)', alignSelf: 'center' }}>
              +{suoi.length - 3}
            </span>
          )}
        </div>
      ...
      */}
      <div style={{
        ...
        borderTop: '1px solid rgba(var(--inchiostro-rgb),.07)'
      }}>
        <span style={{
          width: 6, height: 6, flex: 'none', borderRadius: '50%',
          background: fondo.pallino,
          boxShadow: a.accesa && a.salute.stato === 'bene' ? '0 0 0 3px rgba(var(--salvia-rgb),.2)' : 'none'
        }} />
  ...
  return (
    <Hov as="button" type="button" onClick={apri}
      style={{
        ...
        border: '1px dashed rgba(var(--inchiostro-rgb),.2)', background: 'rgba(var(--carta-rgb),.3)',
        color: 'rgba(var(--inchiostro-rgb),.5)', transition: 'border-color .2s, color .2s, background .2s'
      }}
      hover={{ borderColor: 'rgba(var(--rame-rgb),.6)', color: 'var(--rame-testo)', background: 'rgba(var(--carta-rgb),.55)' }}>
